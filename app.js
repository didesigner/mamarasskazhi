(() => {
  ...
  async function startAR() {
    ...
    try {
      ...
      // iPhone fix: explicitly reveal the camera stream under the AR canvas.
      waitForCameraFeed();
  ...
  function buildARScene(book) {
    ...
    scene.setAttribute("renderer", "colorManagement: true; physicallyCorrectLights: false; alpha: true; antialias: true;");
  ...
  function exposeMindARCameraFeed() {
    // MindAR creates a camera video dynamically.
    // On some iPhones/A-Frame combinations the canvas can cover it with black.
    // Camera video is easy to identify because it has a MediaStream in srcObject.
    const allVideos = Array.from(document.querySelectorAll("video"));
    const cameraVideo = allVideos.find(video => video.srcObject instanceof MediaStream);

    if (!cameraVideo) return false;

    cameraVideo.classList.add("mindar-camera-feed");
    ...
    // Make sure the A-Frame canvas itself is visually transparent.
  ...
  function waitForCameraFeed() {
    let attempts = 0;
    const timer = setInterval(() => {
      attempts += 1;
      if (exposeMindARCameraFeed() || attempts > 40) {
        clearInterval(timer);
      }
    }, 100);
  }
